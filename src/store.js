...
let sampleReducer = (state = {from: 0, until: 0, color: "#00ff00", prop: "", data: []}, action) => {
  if(action.type === "SET_COLOR") return {...state, color: action.color}
  if(action.type === "SET_BORD") return {...state, from: action.obj.from, until: action.obj.until}
  if(action.type === "SET_PROP") return {...state, prop: action.val}
  if(action.type === "SET_DATA") return {...state, data: action.data}
  return state
}
